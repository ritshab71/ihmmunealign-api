...
def get_probability(aln_ums, v_gene):
    ...
    # print(f'1: {prob_mutation_plus_b}')
    nts_addition = (avg_mutations_cr/COMMON_AREA_NUCL_LENGTH) * A_SLOPE * num_nts_cr
    # print(f'2: {nts_addition}')
    a_prob = prob_mutation_plus_b + nts_addition
    return a_prob

def get_a_score_probability(aln_ums, v_gene, offset):
    v_gene_info = count_mutations_v_gene(aln_ums, v_gene)
    cr_v_ratio = 115/len(v_gene) # todo: define 115 real length
    avg_mutations_cr = v_gene_info.num_mutations_v_gene * cr_v_ratio

    num_mutations_cr = 0
    num_nts_cr = 0

    start_index = COMMON_AREA_START_POS - 1 - offset
    end_index = COMMON_AREA_END_POS - 1 - offset
